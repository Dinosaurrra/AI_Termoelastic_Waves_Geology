...
The complete preprocessing workflow is summarized in Figure~\ref{fig:dataset_preprocessing_pipeline}.
...
\node[process] (raw) {Raw COMSOL\\exports};
\node[process, right=0.8cm of raw] (cleaning) {CSV parsing\\and cleaning};
\node[process, right=0.8cm of cleaning] (alignment) {Coordinate\\alignment};
\node[process, right=0.8cm of alignment] (features) {Dynamic/static\\feature separation};
\node[process, right=0.8cm of features] (tensor) {State tensor\\construction};
\node[process, right=0.8cm of tensor] (normalization) {Feature\\normalization};

\node[artifact, below=1.2cm of normalization] (artifacts) {Model-ready\\processed artifacts};
\node[process, right=0.8cm of artifacts] (models) {Neural surrogate\\models};

\node[process, below=1.2cm of alignment] (graph) {Graph\\construction};
\node[artifact, right=0.8cm of graph] (graphartifacts) {Graph artifacts\\and trajectories};
\node[process, right=0.8cm of graphartifacts] (graphmodels) {Graph-based\\models};

\draw[arrow] (raw) -- (cleaning);
\draw[arrow] (cleaning) -- (alignment);
\draw[arrow] (alignment) -- (features);
\draw[arrow] (features) -- (tensor);
\draw[arrow] (tensor) -- (normalization);
\draw[arrow] (normalization) -- (artifacts);
\draw[arrow] (artifacts) -- (models);

\draw[dashedarrow] (alignment) -- (graph);
\draw[arrow] (graph) -- (graphartifacts);
\draw[arrow] (graphartifacts) -- (graphmodels);

\end{tikzpicture}
}
\caption{Dataset preprocessing pipeline from raw COMSOL exports to model-ready representations for neural surrogate models. The dashed branch denotes the additional graph construction step used by graph-based architectures.}
\label{fig:dataset_preprocessing_pipeline}
\end{figure}
