...
The simulated physical system consists of a three-dimensional rock domain and a heated metallic rod partially inserted into the rock. The rock represents the geological medium, while the rod acts as a localized thermal excitation source. The size of the rock domain is denoted as
...
The first part is the thermal physics. The \textit{Heat Transfer in Solids} interface was used to solve transient heat conduction in both the rock and the rod. This interface computes the temperature field \(T(\mathbf{x},t)\) over the computational domain.
...
where \(\mathbf{q}\) is the heat flux vector, \(k\) is the thermal conductivity, and \(T\) is the temperature field. The negative sign means that heat flows from hotter regions to colder regions. Therefore, the heat flux is directed opposite to the temperature gradient.
...
\[
\rho = \rho(\mathbf{x}), \qquad
C_p = C_p(\mathbf{x}), \qquad
k = k(\mathbf{x}),
\]
...
where \(\alpha\) is the coefficient of thermal expansion, \(T_{\mathrm{ref}}\) is the reference temperature, and \(\mathbf{I}\) is the identity tensor. Thermal strain describes expansion or contraction of the material due to heating or cooling.

The elastic strain is the part of the total strain that contributes to mechanical stress:

\[
\boldsymbol{\varepsilon}_{\mathrm{el}}
=
\boldsymbol{\varepsilon}
-
\boldsymbol{\varepsilon}_{\mathrm{th}}.
...
where \(\boldsymbol{\sigma}\) is the Cauchy stress tensor and \(\mathbf{C}\) is the elasticity tensor. For an isotropic linear elastic material, this relation can be written as
...
where \(\rho\) is the density and \(\mathbf{f}\) is the body force vector. This equation describes how stress gradients produce acceleration of the material. The divergence of the stress tensor acts as an internal force, while the inertial term controls wave-like propagation of displacement.

\subsubsection{Thermoelastic coupling}

The thermoelastic coupling is the central physical mechanism of the simulation. The heat transfer interface computes the temperature field \(T(\mathbf{x},t)\). This temperature field is then passed to the solid mechanics interface. COMSOL uses the temperature difference relative to the reference temperature to compute the thermal strain:
...
T(\mathbf{x},t)
\longrightarrow
\boldsymbol{\varepsilon}_{\mathrm{th}}(\mathbf{x},t)
\longrightarrow
...
where \(\mathbf{n}\) is the outward normal vector to the boundary. This boundary condition means that no heat leaves the model through the selected external surfaces. It is useful when the goal is to study internal heat propagation inside the geological volume.
...
where \(\mathbf{x}_i\) are node coordinates. For each edge between neighboring nodes \(i\) and \(j\), edge attributes can be defined as
...
\qquad
\boldsymbol{\sigma}(\mathbf{x},t_n),
\qquad
\boldsymbol{\varepsilon}(\mathbf{x},t_n).
\]
...
The COMSOL simulation exports the physical fields required for training and validating AI models. The exported variables include thermal variables, mechanical variables, and material parameters. The thermal variables include temperature \(T\), thermal conductivity \(k\), density \(\rho\), and heat capacity \(C_p\). The mechanical variables include displacement components \(u\), \(v\), \(w\), velocity components \(u_t\), \(v_t\), \(w_t\), von Mises stress \(\sigma_{\mathrm{vM}}\), stress tensor components \(\sigma_x\), \(\sigma_y\), \(\sigma_z\), \(\sigma_{xy}\), \(\sigma_{yz}\), \(\sigma_{xz}\), and strain components \(\varepsilon_x\), \(\varepsilon_y\), \(\varepsilon_z\). The material parameters include Young's modulus \(E\), Poisson's ratio \(\nu\), solid density \(\rho_s\), and thermal expansion coefficient \(\alpha\).
...
Displacement & \(u\), \(v\), \(w\) & Mechanical displacement components \\
Velocity & \(u_t\), \(v_t\), \(w_t\) & Time derivatives of displacement components \\
Stress & \(\sigma_{\mathrm{vM}}\), \(\sigma_x\), \(\sigma_y\), \(\sigma_z\), \(\sigma_{xy}\), \(\sigma_{yz}\), \(\sigma_{xz}\) & Stress field components \\
Strain & \(\varepsilon_x\), \(\varepsilon_y\), \(\varepsilon_z\) & Strain field components \\
...
where \(\mathbf{s}(t)\) is the current physical state, \(\mathbf{p}\) contains material and source parameters, \(\mathbf{x}\) denotes spatial coordinates, and \(\mathbf{s}(t+\Delta t)\) is the future physical state.

For MeshGraphNet, the prediction for node \(i\) can be written as

\[
\hat{\mathbf{s}}_i(t+\Delta t)
=
f_{\theta}
\left(
\mathbf{s}_i(t),
...
\mathbf{p}_i,
\left\{
\mathbf{s}_j(t), \mathbf{e}_{ij}
\right\}_{j \in \mathcal{N}(i)}
\right),
\]

where \(f_{\theta}\) is the neural network model, \(\mathcal{N}(i)\) is the set of neighboring nodes of node \(i\), and \(\mathbf{e}_{ij}\) are edge features. This formulation is consistent with MeshGraphNet because physical processes in a solid medium are local: the future state of a node depends on its own state, material parameters, and neighboring nodes.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/comsol_geometry_placeholder.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/comsol_mesh_placeholder.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/comsol_temperature_placeholder.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/comsol_stress_placeholder.png}
\caption{Stress field generated by non-uniform thermal expansion.}
\label{fig:comsol_stress}
\end{figure}

Figure~\ref{fig:comsol_dataset} shows the exported dataset structure used for neural network training. It connects the COMSOL simulation stage with the later machine learning pipeline.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{figures/comsol_dataset_placeholder.png}
\caption{Exported dataset structure used for neural network training.}
\label{fig:comsol_dataset}
\end{figure}
...
Limited computational resources restricted the complexity and number of simulated scenarios. For example, very large three-dimensional models, very fine meshes, and many parameter combinations require significant memory and computation time. However, these limitations were controlled by keeping the setup consistent and by focusing on physically meaningful scenarios.
