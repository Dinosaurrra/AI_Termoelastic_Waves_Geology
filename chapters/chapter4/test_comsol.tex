...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{chapters/chapter4/interface.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{chapters/chapter4/object.png}
...
\begin{table}[H]
\centering
\caption{Main COMSOL physics interfaces and model components.}
\label{tab:comsol_physics_interfaces}
\begin{tabular}{p{0.28\textwidth} p{0.62\textwidth}}
\hline
\textbf{COMSOL component} & \textbf{Role in the simulation} \\
\hline
\textit{Solid Mechanics} & Computes displacement, velocity, strain, stress, and the mechanical response of the rock caused by thermal expansion. \\
\hline
\textit{Linear Elastic Material} & Defines the elastic behavior of the geological medium using material parameters such as Young's modulus, Poisson's ratio, density, and thermal expansion coefficient. \\
\hline
\textit{Free Boundary} & Allows selected boundaries to deform naturally without externally applied mechanical traction. \\
\hline
\textit{Heat Transfer in Solids} & Computes transient heat conduction inside the rock and rod domains. \\
\hline
\textit{Thermal Insulation} & Prevents heat flux through selected external boundaries. \\
\hline
\textit{Thermal Expansion} & Couples the temperature field with the mechanical model by converting temperature changes into thermal strain. \\
\hline
\textit{Temperature Coupling} & Transfers the temperature solution from the heat transfer interface to the solid mechanics interface. \\
\hline
\textit{Time Dependent Study} & Solves the coupled system over a sequence of time steps and produces time-dependent physical fields. \\
\hline
\end{tabular}
\end{table}
