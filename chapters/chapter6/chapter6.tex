...
is stored in
\texttt{ai-directional-prediction/granite-analytics/outputs/} and is
the source of every plot and number in this chapter.
...
The fourth metric is the directional response itself, reported as the
predicted azimuth and elevation of the propagation vector. The
azimuth comparison is summarised in Figure~\ref{fig:azimuth} and the
elevation in Figure~\ref{fig:elevation}. Three of the four routes
agree to within roughly two degrees on the azimuth: PINN, MeshGraphNet
and the Transformer all return values in the $11$--$13^{\circ}$ band,
which is close to the geometric source-to-probe direction defined in
the request payload. The Fourier neural operator, however, returns an
azimuth of approximately $-167^{\circ}$, which is geometrically
implausible and is again attributable to its 2D adaptation: the
current FNO route projects three-dimensional scenarios into two
dimensions in a way that produces an inverted azimuthal sign.
...
\end{axis}
\end{tikzpicture}
\caption{Predicted azimuth of the propagation vector. Three of the
four routes agree to within two degrees and align with the geometric
source-to-probe direction; the Fourier neural operator produces an
inverted azimuth as a consequence of its current 2D adaptation.}
\label{fig:azimuth}
\end{figure}
...
\addplot+[fill=purple!30, draw=purple!60!black] coordinates {(PINN,45.2) (MeshGraphNet,38.1) (Transformer,59.6) (FNO,0.0)};
\end{axis}
\end{tikzpicture}
\caption{Predicted elevation of the propagation vector. PINN,
MeshGraphNet and the Transformer-based operator all produce non-zero
elevations, which confirms that they treat the scenarios as
three-dimensional. The Fourier neural operator collapses the
elevation to zero because of its rect\_3d $\to$ rect\_2d adaptation.}
\label{fig:elevation}
\end{figure}

The elevation diagnostic separates the three-dimensional predictors
from the FNO route in a particularly clear way. PINN, MeshGraphNet and
the Transformer all return non-zero elevation angles, which means
that their predicted direction vector has a non-zero $z$-component
and is therefore a genuine three-dimensional output. The Fourier
neural operator returns an elevation of exactly zero because every
3D query is internally collapsed to a 2D field before its prediction
is read. We therefore distinguish, in the rest of this discussion,
between the three \emph{3D-capable routes} (PINN, MeshGraphNet,
Transformer) and the single \emph{2D-only route} (FNO) in its current
...
reports the mean per-request latency measured on the host machine
during the same $320$-response sweep. All four routes deliver an
answer in well under one second and are therefore compatible with
interactive use through the frontend of Chapter~4. The PINN baseline
is the fastest at $\approx 30$\,ms because it is essentially a
shallow point-wise multilayer perceptron evaluated once per probe.
The Transformer-based operator is next at $\approx 110$\,ms; its cost
scales quadratically with the chosen token count, and the subsampling
to $N_{\mathrm{in}}=N_q=1024$ keeps it well within the interactive
regime. The Fourier neural operator costs $\approx 180$\,ms,
dominated by the per-layer fast Fourier transform. MeshGraphNet is
the slowest at $\approx 240$\,ms because it runs an autoregressive
rollout of five message-passing steps on the full mesh graph.
...
\begin{axis}[
  ...
  xbar, bar width=14pt,
  ymajorgrids, grid style={black!10},
  xlabel={Mean inference latency [ms / request]},
  xlabel near ticks,
  symbolic y coords={MeshGraphNet, FNO, Transformer, PINN},
  ytick=data,
  nodes near coords,
  nodes near coords style={font=\scriptsize, /pgf/number format/.cd, fixed, fixed zerofill, precision=0},
  xmin=0, xmax=320
]
\addplot+[fill=cyan!30, draw=cyan!60!black] coordinates {(30,PINN) (110,Transformer) (180,FNO) (240,MeshGraphNet)};
\end{axis}
\end{tikzpicture}
\caption{Mean per-request inference latency measured during the
$320$-response sweep on the host machine. All four routes are
interactive; the PINN baseline is the cheapest and MeshGraphNet is the
most expensive because of its autoregressive rollout.}
\label{fig:latency}
\end{figure}
...
of $\approx 0.74$, consistent with the metric file
\texttt{pinn-service/artifacts/checkpoints/baseline/metrics.json}.
...
flat for PINN and MeshGraphNet. The directional metrics (azimuth and
elevation) are essentially material-blind: each route returns the
same direction for both materials to within a fraction of a degree.
...
3D-capable until its denormalisation and dimensionality issues are
...
three-dimensional directional output; at the price of being the
...
neural operator, in its present 2D adaptation, cannot yet be ranked
on a comparable basis: it produces non-physical scales on the
displacement and temperature axes and a degenerate elevation of
zero, and a 3D denormalisation upgrade is the first concrete
implementation task identified by this evaluation.
...
FNO~3D variant with consistent output normalisation is implemented.
