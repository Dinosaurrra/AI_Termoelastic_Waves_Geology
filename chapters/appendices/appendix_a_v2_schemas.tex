\section{API contract v2: Pydantic request schemas}
\addcontentsline{toc}{section}{Appendix A. API contract v2 schemas}

This appendix shows the Pydantic v2 schemas that implement the client-
facing v2 request contract described in Chapter~5. The schemas live in
\path{backend/app/schemas/prediction.py}; only the v2 subset is
reproduced here. The full file also contains the unchanged v1 schemas
that remain valid through schema-version dispatch.

\begin{lstlisting}[language=Python,caption={V2 request schemas — \texttt{backend/app/schemas/prediction.py}}]
...
The four \texttt{Literal[...]} fields combined with \texttt{extra="forbid"}
implement the rejection rules of Chapter~5: any client field for
\texttt{thermal\_state}, \texttt{frequency\_hz}, or \texttt{domain.size}
is refused with a \texttt{schema\_validation\_failed} error. The
co-incidence check in \texttt{Geometry2DSchema} prevents division by
zero in the propagation-vector derivation of Appendix~B.
