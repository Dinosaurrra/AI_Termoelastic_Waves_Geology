\section{Derived geometric and elastic quantities}
\addcontentsline{toc}{section}{Appendix B. Derived quantities}
...
--- is computed by a single module
\path{backend/app/domain/services/derived\_quantities.py}.
Keeping all formulas in one file is intentional: the contract document
points at this module as the single source of truth, and unit tests in
\path{backend/tests/test_derived\_quantities.py} pin every formula to a
known reference value. The module is pure Python with no FastAPI or
Pydantic dependencies.

\begin{lstlisting}[language=Python,caption={Excerpt of \texttt{derived\_quantities.py}}]
