...
The mathematical formulation in Chapter 2 describes thermoelastic wave propagation through a set of coupled physical fields and material parameters. In its complete form, the problem involves the temperature field \(T(\mathbf{x},t)\), the temperature perturbation \(\theta(\mathbf{x},t)\), the displacement field \(\mathbf{u}(\mathbf{x},t)\), the strain tensor \(\varepsilon_{ij}\), the stress tensor \(\sigma_{ij}\), and the material parameters of the geological medium. These quantities are connected through the equation of motion, the strain--displacement relation, the thermoelastic constitutive relation, and the coupled heat-conduction equation.

For the purposes of this thesis, this formulation is interpreted as the physical basis of a prediction task. Instead of claiming to solve the complete coupled partial differential equation system for every point in a three-dimensional field-scale geological domain, the methodology defines a structured mapping from a set of physically meaningful inputs to a set of predicted thermoelastic response quantities. In general form, this can be written as
\begin{equation}
\mathcal{F}:
\mathbf{X}
\rightarrow
