...
T,\theta,\mathbf{x}_s,\mathbf{x}_p,\hat{\mathbf{d}},t
\right],
\end{equation}
where \(m\) denotes the material type, \(\rho\) is density, \(E\), \(\nu\), \(\mu\), \(K\), and \(\lambda\) are elastic parameters, \(\alpha\), \(k\), \(C_p\), \(C\), and \(\gamma\) are thermal and thermoelastic parameters, \(T\) and \(\theta\) describe the thermal state, \(\mathbf{x}_s\) and \(\mathbf{x}_p\) are the source and probe locations, \(\hat{\mathbf{d}}\) is the unit propagation direction, and \(t\) denotes time or an observation setting.
...
The geometric part of the input describes direction. The source point \(\mathbf{x}_s\) and probe point \(\mathbf{x}_p\) determine the propagation vector
\begin{equation}
\mathbf{d}
=
...
\end{equation}
This source--probe representation allows the response to be interpreted along selected propagation directions. It is also compatible with simplified two-dimensional sections or local cross-sections when a full three-dimensional field is not being computed.
