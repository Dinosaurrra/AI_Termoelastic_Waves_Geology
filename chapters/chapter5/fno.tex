...
\mathcal{G}_{\theta}(a),
\]
where \(a\) is the input tensor and \(\hat{\mathbf{s}}\) is the predicted thermoelastic field.
...
C_{out}=4,
\]
because the model predicts four output fields:
\[
...
\hat{\mathbf{S}}
\in
\mathbb{R}^{B \times 4 \times N_x \times N_y}.
\]
...
[B, 4, N_x, N_y].
\]
The model first lifts the input to a hidden representation, optionally appends coordinate grid information, applies a sequence of Fourier blocks, and then projects the latent field to four output channels:
\[
T, \quad u, \quad v, \quad w.
\]
