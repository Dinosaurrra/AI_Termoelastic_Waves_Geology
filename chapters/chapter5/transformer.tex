...
The coupled thermoelastic system written down in Chapter~3 defines a
...
COMSOL solver of Chapter~5. Three properties of the present problem
...
construction discretisation-invariant (Figure~\ref{fig:transformer-tokens}).
...
\node[
  ...
  text width=60mm,
  font=\scriptsize
] at ([yshift=1mm]inset.north)
{Input set\\(mesh nodes, ground-truth state at \(t=0\))};
...
\node[
  ...
  text width=38mm,
  font=\scriptsize
] at ([yshift=1mm]qset.north)
{Query set\\(arbitrary points)};

\end{tikzpicture}
\caption{Tokenisation of the thermoelastic state}
...
\end{equation}
Within the framework of Chapter~3 these three channels coincide with
...
coupled thermoelastic problem of Chapter~5. The primary supervised
...
the heat equation derived in Chapter~3,
\begin{equation}
...
such as the rod-in-rock setup of Chapter~5. We treat the plain
...
Vanilla Transformer \cite{vaswani2017}
& \(\mathcal{O}(N^2 d_m)\)
& Softmax dot-product on sequences \\
\hline

OFormer \cite{liNeuralOperator2023}
& \(\mathcal{O}(N_{\mathrm{in}} N_q d_m)\)
& Cross-attention from input set to query set \\
\hline

Galerkin Transformer \cite{caoChooseTransformer2021}
& \(\mathcal{O}(N d_m^2)\)
& Softmax replaced by Galerkin-style linear attention \\
\hline

Transolver \cite{wuTransolver2024}
& \(\mathcal{O}(N M d_m)\)
& Latent physics-aware slots with \(M\!\ll\!N\) \\
\hline

This work
& \(\mathcal{O}(N_{\mathrm{in}} N_q d_m)\)
& OFormer baseline, \(N_{\mathrm{in}}\!=\!N_q\!=\!1024\) subsampling \\
\hline

\end{tabularx}

\caption{Position of the Transformer-based neural operator among set-based operators}
...
complement to the Fourier neural operator of Chapter~8, which is
intrinsically tied to a regular grid, and to the MeshGraphNet of
Chapter~9, which is intrinsically tied to a particular graph
topology.
...
of Chapter~3 trained on the COMSOL ground-truth solutions of
Chapter~5. It tokenises the medium into input points with embedded
...
MeshGraphNet routes evaluated in the same comparison framework.
