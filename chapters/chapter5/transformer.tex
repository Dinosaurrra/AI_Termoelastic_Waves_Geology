\subsection{Transformer-Based Neural Operator}

\subsubsection{Motivation for an attention-based surrogate}

The coupled thermoelastic system written down in Chapter~2 defines a
deterministic mapping
\[
\mathcal{G}: (a,\,\mathrm{IC},\,\mathrm{BC},\,s) \;\longmapsto\; \big(T(\mathbf{x},t),\,\mathbf{u}(\mathbf{x},t),\,\sigma(\mathbf{x},t)\big),
\]
where the input collects the material parameter field
\(a(\mathbf{x}) = (\rho,\lambda,\mu,\gamma,k,C_p)\), the initial and
...
\(\mathbf{x}_i\in\Omega\) we form an input token whose feature vector
...
\big[\,
  ...
  T_i^{(0)},\;
  u_i^{(0)},\,v_i^{(0)},\,w_i^{(0)},\;
  \dot u_i^{(0)},\,\dot v_i^{(0)},\,\dot w_i^{(0)},\;
  E_i,\,\nu_i,\,\rho_i,\,\alpha_i,\,k_i,\,C_{p,i}
\,\big]\in\mathbb{R}^{16}.
\end{equation}
The first three entries fix the location at which the token is
attached, the next seven entries describe the state of the rock at the
...
\begin{equation}
q_j = (x_q^{(j)},\,y_q^{(j)},\,z_q^{(j)})\in\mathbb{R}^{3}.
\end{equation}
...
\node[itok] (i1) {\(a_1 = (\mathbf{x}_1,\;T_1^{(0)},\;\mathbf{u}_1^{(0)},\;\dot{\mathbf{u}}_1^{(0)},\;a_1^{\text{mat}})\)};
\node[itok, below=of i1] (i2) {\(a_2 = (\mathbf{x}_2,\;\ldots)\)};
\node[below=2mm of i2] (idots) {\(\vdots\)};
\node[itok, below=2mm of idots] (iN) {\(a_{N_{\mathrm{in}}} = (\mathbf{x}_{N_{\mathrm{in}}},\;\ldots)\)};

\node[draw=black!50, dashed, fit=(i1) (iN), inner sep=4mm, label=above:{\textbf{Input set} (mesh nodes, ground-truth state at \(t=0\))}] (inset) {};

\node[qtok, right=18mm of i1.east] (q1) {\(q_1 = (\mathbf{x}_q^{(1)})\)};
\node[qtok, below=of q1] (q2) {\(q_2 = (\mathbf{x}_q^{(2)})\)};
\node[below=2mm of q2] (qdots) {\(\vdots\)};
\node[qtok, below=2mm of qdots] (qN) {\(q_{N_q} = (\mathbf{x}_q^{(N_q)})\)};

\node[draw=black!50, dashed, fit=(q1) (qN), inner sep=4mm, label=above:{\textbf{Query set} (arbitrary points)}] (qset) {};
...
representation \(\mathbf{Q}\in\mathbb{R}^{N_q\times d_k}\), key and
value representations \(\mathbf{K}\in\mathbb{R}^{N_{\mathrm{in}}\times d_k}\)
and \(\mathbf{V}\in\mathbb{R}^{N_{\mathrm{in}}\times d_v}\), the
...
\label{eq:dotproduct}
\end{equation}
The matrix \(\mathbf{Q}\mathbf{K}^{\top}\in\mathbb{R}^{N_q\times N_{\mathrm{in}}}\)
...
\(\mathbf{V}\) returns, for each query, a convex combination of input
...
\operatorname{Attention}\!\big(
  \mathbf{Q}\mathbf{W}^Q_i,\,
  \mathbf{K}\mathbf{W}^K_i,\,
  \mathbf{V}\mathbf{W}^V_i
\big),
\end{equation}
\begin{equation}
\operatorname{MultiHead}(\mathbf{Q},\mathbf{K},\mathbf{V}) =
\operatorname{Concat}(\operatorname{head}_1,\dots,\operatorname{head}_h)\,\mathbf{W}^O,
\end{equation}
where the projection matrices satisfy
\(\mathbf{W}^Q_i,\mathbf{W}^K_i,\mathbf{W}^V_i\in\mathbb{R}^{d_m\times d_k}\)
and \(\mathbf{W}^O\in\mathbb{R}^{h d_v\times d_m}\). In the baseline
...
\(\mathbf{W}_{\mathrm{in}}\in\mathbb{R}^{C_{\mathrm{in}}\times d_m}\),
yielding initial representations
\(\mathbf{H}^{(0)}\in\mathbb{R}^{N_{\mathrm{in}}\times d_m}\) with
\(C_{\mathrm{in}}=16\). They are then refined by a stack of
...
\operatorname{LayerNorm}\!\big(
  \mathbf{H}^{(\ell)} +
  \operatorname{MultiHead}(\mathbf{H}^{(\ell)},\mathbf{H}^{(\ell)},\mathbf{H}^{(\ell)})
\big),
\end{equation}
\begin{equation}
\mathbf{H}^{(\ell+1)} =
...
\mathbf{W}_2\,\phi\!\big(\mathbf{W}_1\mathbf{h} + \mathbf{b}_1\big) + \mathbf{b}_2,
\end{equation}
where \(\mathbf{W}_1\in\mathbb{R}^{d_{\mathrm{ff}}\times d_m}\),
\(\mathbf{W}_2\in\mathbb{R}^{d_m\times d_{\mathrm{ff}}}\), and the
...
\(\mathbf{H}\in\mathbb{R}^{N_{\mathrm{in}}\times d_m}\) that captures
how each input token relates to every other input token through the
learnt self-attention pattern.

\subsubsection{Cross-attention decoder for query points}
...
\(\mathbf{W}_{\mathrm{q}}\in\mathbb{R}^{3\times d_m}\) into a query
embedding \(\mathbf{Z}^{(0)}\in\mathbb{R}^{N_q\times d_m}\). A stack of
...
\operatorname{LayerNorm}\!\big(
  \mathbf{Z}^{(\ell)} +
  \operatorname{MultiHead}(\mathbf{Z}^{(\ell)},\mathbf{H},\mathbf{H})
\big),
\end{equation}
\begin{equation}
\mathbf{Z}^{(\ell+1)} =
\operatorname{LayerNorm}\!\big(
  \mathbf{Z}^{(\ell)'} +
  \operatorname{FFN}(\mathbf{Z}^{(\ell)'})
\big).
\end{equation}
After the final decoder layer a linear output head
\(\mathbf{W}_{\mathrm{out}}\in\mathbb{R}^{d_m\times C_{\mathrm{out}}}\)
maps every decoded query token to a vector of predicted physical
quantities:
\begin{equation}
\hat{y}_j =
\big(\hat T_j,\,\hat u_j,\,\hat v_j,\,\hat w_j\big) =
\mathbf{W}_{\mathrm{out}}\,\mathbf{Z}^{(L_{\mathrm{dec}})}_{j,:}
\in\mathbb{R}^{C_{\mathrm{out}}},
\qquad C_{\mathrm{out}} = 4.
\end{equation}
Within the framework of Chapter~2 these four channels coincide with
the primary unknowns of the thermoelastic system: the temperature
field and the three components of the displacement vector. The other
...
\subsubsection{Architecture used in this work}
...
\node[inp] (in)   {Input tokens \(\{a_i\}\in\mathbb{R}^{16}\)};
\node[inp, below=of in] (ip) {Input projection \(\mathbb{R}^{16}\!\to\!\mathbb{R}^{d_m}\)};
...
\node[qry, right=24mm of in] (q)  {Query tokens \(\{q_j\}\in\mathbb{R}^{3}\)};
\node[qry, below=of q]   (qp) {Query projection \(\mathbb{R}^{3}\!\to\!\mathbb{R}^{d_m}\)};
...
\node[outp, below=18mm of $(eL)!0.5!(dL)$] (head) {Linear head \(\mathbb{R}^{d_m}\!\to\!\mathbb{R}^{4}\)};
\node[outp, below=of head] (yhat) {\(\hat{y}_j = (\hat T_j,\,\hat u_j,\,\hat v_j,\,\hat w_j)\)};
...
\draw[arr] (eL.east) -- ++(8mm,0) |- (d1.west) node[pos=0.25, right] {\scriptsize \(\mathbf{K},\,\mathbf{V}\)};
...
head maps the decoded query embeddings to the four predicted physical
...
\big\|\,\partial_t \hat{\mathbf{u}}_j - \dot{\mathbf{u}}_j^{\mathrm{ref}}\big\|_2^2,
\label{eq:vel}
\end{equation}
where the time derivative \(\partial_t\hat{\mathbf{u}}_j\) is obtained
...
\subsubsection{Comparison with related Transformer neural operators}
...
\begin{tabular}{l l l}
\toprule
\textbf{Architecture} & \textbf{Attention cost} & \textbf{Key idea} \\
\midrule
...
\midrule
\textbf{This work} & \(\mathcal{O}(N_{\mathrm{in}} N_q d_m)\) & OFormer baseline, \(N_{\mathrm{in}}\!=\!N_q\!=\!1024\) subsampling \\
\bottomrule
\end{tabular}
...
\subsubsection{Discretisation invariance and operator-learning view}
