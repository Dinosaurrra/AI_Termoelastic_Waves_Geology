...
equations introduced in Chapter~2. The other three routes (FNO,
...
follow Section~2.4. The output triple $(T,u,v)$ contains the
...
\textbf{Supervised loss} $\mathcal{L}_{\mathrm{sup}}$ is the standard
mean-squared error between the predicted $(T,u,v)$ and the
ground-truth values exported from COMSOL at the corresponding
$(x_i,y_i,t_i)$.

\textbf{Velocity consistency} $\mathcal{L}_{\mathrm{vel}}$ enforces
...
\textbf{Elastic-wave residual} $\mathcal{L}_{\mathrm{elastic}}$
...
\textbf{Coupled-thermal residual} $\mathcal{L}_{\mathrm{thermal}}$
...
Section~2.5. Including it makes the PINN sensitive to the geometric
...
Chapter~4 exported from COMSOL ($230$ nodes per material,
...
comparison results of Chapter~6 will show that this is the only route
