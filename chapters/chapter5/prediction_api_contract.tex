...
The directional part of the contract follows directly from the mathematical formulation. If the source position is denoted by \(x_s\) and the probe position by \(x_p\), then the propagation vector is
\begin{equation}
d
=
x_p - x_s,
...
\frac{d}{\|d\|}.
\end{equation}
From this vector, directional quantities such as propagation distance, azimuth, and elevation can be computed. In two-dimensional notation, if \(d=(d_x,d_y,d_z)\), then the azimuth is given by
\begin{equation}
...
Another required input group is the domain description. In the current backend, this is represented by the \texttt{domain} object with subfields such as \texttt{type}, \texttt{size}, \texttt{resolution}, and boundary conditions. At the theoretical level, Chapter~2 is formulated in general two-dimensional form, so a general contract may allow both \texttt{rect\_2d} and \texttt{rect\_2d}. However, the practical thesis experiments are prototype experiments and are primarily based on \texttt{rect\_2d} domains. The backend validator explicitly requires \texttt{lz = 0} and \texttt{nz = 1} when \texttt{domain.type = rect\_2d}. This is a useful implementation detail because it encodes the practical scope directly in the API contract: the general request shape remains compatible with future two-dimensional extensions, but the current experiments operate on planar cross-sections or selected directional paths.
