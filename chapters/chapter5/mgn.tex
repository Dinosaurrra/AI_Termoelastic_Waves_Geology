...
where \(u\) is the displacement vector, and \(u\), \(v\), and \(w\) are displacement components along the three spatial directions.
...
where \(\sigma\) is the stress tensor, \(\sigma_x\), \(\sigma_y\), and \(\sigma_z\) are normal stress components, and \(\sigma_{xy}\), \(\sigma_{xz}\), and \(\sigma_{yz}\) are shear stress components.
