...
u_0(x),
\end{equation}
where \(u_0(x)\) is the initial displacement distribution. The initial velocity may be written as
\begin{equation}
\frac{\partial u}{\partial t}(x,0)
=
v_0(x),
\end{equation}
where \(v_0(x)\) is the initial velocity field. For the thermal field, the initial temperature distribution is given by
\begin{equation}
T(x,0)
=
T_{\mathrm{init}}(x).
...
\text{on } \Gamma_t ,
\end{equation}
where \(n\) is the outward unit normal vector to the boundary and \(\bar{t}_i\) is the prescribed traction component.
