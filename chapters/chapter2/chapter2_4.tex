...
The elastic response of the medium is described by elastic moduli. Young's modulus \(E\) measures the resistance of the material to axial deformation under uniaxial loading. It has units of pressure, \(\mathrm{Pa}\). A rock with a higher Young's modulus undergoes smaller axial strain under the same applied stress, while a more compliant rock deforms more easily. Poisson's ratio \(\nu\) is dimensionless and describes the ratio between lateral and axial deformation. It determines how much the material expands laterally when compressed or contracts laterally when stretched. Together, \(E\) and \(\nu\) can be used to derive the other elastic constants in the isotropic linear elastic approximation \cite{jaeger2007,mavko2009}.
...
The thermoelastic coupling coefficient \(\gamma\) connects the thermal and mechanical parts of the formulation. For an isotropic solid, it is commonly written as
\begin{equation}
\gamma = 3K\alpha.
\end{equation}
