...
The heat equation is often interpreted as a diffusion equation. This means that a localized temperature disturbance does not propagate like an elastic wave with a sharply defined wavefront. Instead, the disturbance spreads gradually as heat diffuses through the medium. The rate of this spreading is controlled by the thermal diffusivity $\alpha_T = k/(\rho C_p)$ already introduced in Eq.~\eqref{eq:thermal-diff},
