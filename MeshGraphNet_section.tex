...
where \(f_{\theta}\) is the trainable MeshGraphNet model with parameters \(\theta\), \(\mathbf{X}^{t}\) is the matrix of input node features at time \(t\), \(\mathcal{G}\) is the graph representation of the finite-element mesh, and \(\hat{\mathbf{Y}}^{t+\Delta t}\) is the predicted matrix of physical fields at the next time step \(t+\Delta t\).
...
Each node has a coordinate vector in three-dimensional space:
...
where \(\mathbf{p}_i\) is the position vector of node \(i\), and \(x_i\), \(y_i\), and \(z_i\) are the Cartesian coordinates of this node.

For each edge \((i,j)\), the relative geometric vector is defined as

\[
\mathbf{r}_{ij} = \mathbf{p}_j - \mathbf{p}_i
\]

where \(\mathbf{r}_{ij}\) is the vector directed from node \(i\) to node \(j\), \(\mathbf{p}_i\) is the coordinate vector of the source node, and \(\mathbf{p}_j\) is the coordinate vector of the neighboring destination node.
...
where \(d_{ij}\) is the Euclidean distance between nodes \(i\) and \(j\), \(\mathbf{r}_{ij}\) is the relative position vector, and \(\|\cdot\|_2\) denotes the Euclidean norm.
...
where \(\mathbf{a}_{ij}\) is the feature vector of edge \((i,j)\), \(\mathbf{r}_{ij}\) describes the direction and relative displacement between neighboring nodes, and \(d_{ij}\) describes the distance between these nodes.
...
where \(\mathbf{q}_i^t\) is the physical state vector of node \(i\) at time \(t\), \(T_i^t\) is the temperature, \(u_i^t\), \(v_i^t\), and \(w_i^t\) are displacement components, \(\dot{u}_i^t\), \(\dot{v}_i^t\), and \(\dot{w}_i^t\) are velocity components, \(\sigma_{x,i}^t\), \(\sigma_{y,i}^t\), \(\sigma_{z,i}^t\), \(\sigma_{xy,i}^t\), \(\sigma_{yz,i}^t\), and \(\sigma_{xz,i}^t\) are stress components, and \(\varepsilon_{x,i}^t\), \(\varepsilon_{y,i}^t\), and \(\varepsilon_{z,i}^t\) are strain components.

The temperature component \(T_i^t\) describes the thermal state of the medium at node \(i\). The displacement components \(u_i^t\), \(v_i^t\), and \(w_i^t\) describe mechanical movement in three spatial directions. The velocity components \(\dot{u}_i^t\), \(\dot{v}_i^t\), and \(\dot{w}_i^t\) describe the rate of this movement. The stress components \(\sigma_{x,i}^t\), \(\sigma_{y,i}^t\), \(\sigma_{z,i}^t\), \(\sigma_{xy,i}^t\), \(\sigma_{yz,i}^t\), and \(\sigma_{xz,i}^t\) describe internal mechanical forces. The strain components \(\varepsilon_{x,i}^t\), \(\varepsilon_{y,i}^t\), and \(\varepsilon_{z,i}^t\) describe local deformation of the material.
...
where \(\mathbf{X}^{t}\) is the full node feature matrix at time \(t\), \(\mathbf{x}_i^t\) is the input feature vector of node \(i\), and \(N\) is the total number of nodes in the graph.
...
=
\begin{bmatrix}
(\mathbf{y}_1^{t+\Delta t})^{\top} \\
(\mathbf{y}_2^{t+\Delta t})^{\top} \\
\vdots \\
(\mathbf{y}_N^{t+\Delta t})^{\top}
\end{bmatrix}
\]

where \(\mathbf{Y}^{t+\Delta t}\) is the target matrix at the next time step, \(\mathbf{y}_i^{t+\Delta t}\) is the target physical field vector for node \(i\), \(N\) is the total number of nodes, and \(\Delta t\) is the time interval between the current and predicted states.

\subsection{Prediction Target}

The objective of ConditionalMeshGraphNet is to predict the future physical state of the geological medium. The model receives the current node features and the graph structure, and then produces predicted physical fields for each node. This prediction can be written as

\[
\hat{\mathbf{y}}_i^{t+\Delta t}
=
f_{\theta}
\left(
\mathbf{x}_i^t,
...
where \(\hat{\mathbf{y}}_i^{t+\Delta t}\) is the predicted output vector for node \(i\) at time \(t+\Delta t\), \(f_{\theta}\) is the trainable MeshGraphNet model, \(\mathbf{x}_i^t\) is the input feature vector of node \(i\) at time \(t\), and \(\mathcal{G}\) is the graph representation of the COMSOL mesh.

The model can be trained to predict either the absolute value of the next physical state or the increment of the state. In the delta prediction mode, the model predicts the change in the physical state:
...
where \(\Delta \hat{\mathbf{q}}_i^t\) is the predicted increment of the physical state at node \(i\), \(f_{\theta}\) is the neural surrogate model, \(\mathbf{x}_i^t\) is the node feature vector at time \(t\), and \(\mathcal{G}\) is the graph structure.

The predicted next state is then reconstructed as
...
where \(\hat{\mathbf{q}}_i^{t+\Delta t}\) is the predicted physical state at the next time step, \(\mathbf{q}_i^t\) is the current physical state, and \(\Delta \hat{\mathbf{q}}_i^t\) is the predicted state increment.

Predicting increments can be more stable than predicting absolute values. In many thermoelastic simulations, the difference between two adjacent time steps is smoother and smaller in magnitude than the full field itself. Therefore, the model focuses on learning the local temporal evolution of the system rather than reconstructing the entire physical state from scratch.

\subsection{Encoder--Processor--Decoder Architecture}

The implemented model, called ConditionalMeshGraphNet, follows the Encoder--Processor--Decoder principle. The full model can be represented as a composition of three main components:
...
where \(f_{\theta}\) is the full graph neural network, \(E_{\theta_E}\) is the encoder with parameters \(\theta_E\), \(P_{\theta_P}\) is the processor with parameters \(\theta_P\), \(D_{\theta_D}\) is the decoder with parameters \(\theta_D\), and \(\circ\) denotes function composition.

The encoder maps raw node and edge features into a latent space. The processor performs several message passing iterations over the graph. The decoder converts the final latent node representations into predicted physical fields.

\subsubsection{Node and Edge Encoders}

The node encoder transforms the original node feature vector into a latent representation:
...
where \(\mathbf{h}_i^{0}\) is the initial latent representation of node \(i\), \(\phi_v^{enc}\) is the node encoder, and \(\mathbf{x}_i\) is the input feature vector of node \(i\).

The edge encoder transforms the edge feature vector into a latent representation:
...
where \(\mathbf{e}_{ij}^{0}\) is the initial latent representation of edge \((i,j)\), \(\phi_e^{enc}\) is the edge encoder, and \(\mathbf{a}_{ij}\) is the edge feature vector.

Both \(\phi_v^{enc}\) and \(\phi_e^{enc}\) are multilayer perceptron networks. They map features with different physical meanings and numerical scales into a common latent dimension. This is necessary because the message passing processor operates not on raw physical values directly, but on learned hidden representations.

The nonlinear activation function used in the multilayer perceptron is the SiLU activation:
...
where \(\operatorname{SiLU}(x)\) is the activation value for input \(x\), and \(\sigma(x)\) is the sigmoid function.
...
The processor consists of several message passing blocks. Let the number of message passing iterations be \(K\). The iteration index is defined as

\[
k = 0,1,\dots,K-1
\]

where \(k\) is the message passing step index, and \(K\) is the total number of message passing iterations.

At each message passing step, the edge features are updated first. The temporary edge update is computed as
...
where \(\tilde{\mathbf{e}}_{ij}^{k+1}\) is the proposed update for edge \((i,j)\), \(\phi_e^{k}\) is the edge update network at message passing step \(k\), \(\mathbf{h}_i^k\) is the latent representation of the source node \(i\), \(\mathbf{h}_j^k\) is the latent representation of the destination node \(j\), and \(\mathbf{e}_{ij}^k\) is the current latent representation of the edge.

After this, a residual connection and layer normalization are applied:
...
where \(\mathbf{e}_{ij}^{k+1}\) is the updated edge representation, \(\mathbf{e}_{ij}^{k}\) is the previous edge representation, \(\tilde{\mathbf{e}}_{ij}^{k+1}\) is the learned edge update, and \(\operatorname{LayerNorm}\) denotes layer normalization.

The edge update models local interaction between two neighboring points of the geological medium. For example, the network can learn how temperature difference, mechanical displacement, stress state, material parameters, and spatial distance influence the transfer of information between nearby nodes.

After updating the edges, messages are aggregated for each destination node:
...
where \(\mathbf{m}_j^{k+1}\) is the aggregated message received by node \(j\), \(\mathbf{e}_{ij}^{k+1}\) is the updated edge representation from node \(i\) to node \(j\), and the summation is performed over all incoming edges \((i,j)\) in the graph \(\mathcal{G}\).

This aggregation corresponds to summing messages from neighboring nodes. In implementation terms, it is analogous to a scatter-add operation over destination nodes. Physically, this means that the future state of a node is influenced by the surrounding local region.

The node update is computed as
...
where \(\tilde{\mathbf{h}}_j^{k+1}\) is the proposed update for node \(j\), \(\phi_v^{k}\) is the node update network at message passing step \(k\), \(\mathbf{h}_j^k\) is the current latent representation of node \(j\), and \(\mathbf{m}_j^{k+1}\) is the aggregated message from neighboring nodes.

The final updated node state is obtained using a residual connection and normalization:
...
where \(\mathbf{h}_j^{k+1}\) is the updated latent representation of node \(j\), \(\mathbf{h}_j^{k}\) is the previous latent representation, \(\tilde{\mathbf{h}}_j^{k+1}\) is the learned node update, and \(\operatorname{LayerNorm}\) is the normalization operation.

Several message passing iterations allow information to propagate over larger regions of the mesh. This is important because thermal diffusion and elastic waves are not purely pointwise processes. They depend on local interactions that spread through the medium over time.

\subsubsection{Decoder}

After \(K\) message passing steps, each node has a final latent representation. The decoder maps this representation to the predicted physical fields:
...
where \(\hat{\mathbf{y}}_i\) is the predicted output vector for node \(i\), \(\phi^{dec}\) is the decoder network, and \(\mathbf{h}_i^K\) is the latent representation of node \(i\) after \(K\) message passing steps.

For the whole graph, the model output can be written as
...
where \(\hat{\mathbf{Y}}\) is the full predicted output matrix, \(\hat{\mathbf{y}}_i\) is the predicted vector for node \(i\), and \(N\) is the number of graph nodes.

\subsection{Physical Interpretation of Message Passing}

The message passing mechanism has a clear physical interpretation in the thermoelastic wave prediction task. In a continuous physical medium, the state of one point depends on the state of neighboring points. Temperature evolves due to heat conduction, while mechanical waves evolve through the interaction between displacement, strain, stress, and material stiffness.

After spatial discretization by the finite-element method, the continuous physical problem becomes a system of local interactions between neighboring mesh nodes or elements. Graph message passing can be interpreted as a data-driven analogue of these local numerical interactions.
...
where \(\mathcal{N}_K(i)\) is the \(K\)-hop neighborhood of node \(i\), \(j\) is a node in the graph, \(\mathcal{V}\) is the set of all nodes, \(\operatorname{dist}_{\mathcal{G}}(i,j)\) is the shortest graph distance between nodes \(i\) and \(j\), and \(K\) is the number of message passing steps.

This formula means that after \(K\) message passing steps, node \(i\) can receive information from all nodes located at a graph distance of at most \(K\) edges. Therefore, increasing \(K\) increases the region of the mesh that can influence each prediction. This is important for wave propagation because a local thermal or mechanical disturbance spreads through the medium over time and affects increasingly distant regions.

\subsection{Loss Function}

The model is trained as a supervised regression model. The predicted physical fields are compared with reference fields obtained from COMSOL simulations. A standard loss function for this task is the mean squared error:
...
where \(\mathcal{L}_{MSE}\) is the mean squared error loss, \(N\) is the number of nodes, \(C\) is the number of predicted physical fields, \(\hat{y}_{i,c}\) is the predicted value of field \(c\) at node \(i\), and \(y_{i,c}\) is the corresponding target value from the simulation.

In this work, the loss function is extended to a weighted version called WeightedFieldMSE. The weighted loss is written as
...
where \(\mathcal{L}\) is the weighted field loss, \(w_c\) is the weight of physical field \(c\), \(C\) is the total number of output fields, \(N\) is the number of nodes, \(\hat{y}_{i,c}\) is the predicted value, and \(y_{i,c}\) is the reference value.

The purpose of weighting is to control the contribution of different physical quantities to the total training objective. In thermoelastic simulations, temperature, displacement, velocity, stress, and strain may have different numerical scales and different importance for the final application. Without weighting, fields with larger numerical magnitudes may dominate the loss, while smaller but physically important fields may be learned poorly.

The field groups can be represented as
...
where \(\mathcal{C}\) is the set of all predicted fields, \(\mathcal{C}_{T}\) is the temperature group, \(\mathcal{C}_{u}\) is the displacement group, \(\mathcal{C}_{\dot{u}}\) is the velocity group, \(\mathcal{C}_{\sigma}\) is the stress group, \(\mathcal{C}_{\varepsilon}\) is the strain group, and \(\mathcal{C}_{other}\) is the group of other fields.
...
where \(w_c\) is the weight of field \(c\), \(w_g\) is the weight assigned to group \(g\), and \(\mathcal{C}_g\) is the set of fields belonging to group \(g\).

\subsection{Evaluation Metrics}

Several metrics are used to evaluate the model. The first metric is the mean squared error:
...
where \(\mathrm{MSE}\) is the mean squared error, \(N\) is the number of nodes, \(C\) is the number of predicted fields, \(\hat{y}_{i,c}\) is the predicted value of field \(c\) at node \(i\), and \(y_{i,c}\) is the target value.
...
In addition to global metrics, per-field metrics are used. The per-field root mean squared error is defined as
...
where \(\mathrm{MAE}_c\) is the mean absolute error for field \(c\), \(N\) is the number of nodes, \(\hat{y}_{i,c}\) is the predicted value, and \(y_{i,c}\) is the target value.

Global metrics are useful for comparing different models or checkpoints using one general number. However, they can hide poor performance on individual physical fields. Per-field metrics are necessary because a model may predict temperature accurately but produce larger errors for stress or strain.

\subsection{Training Procedure}

The training pipeline loads processed datasets from the dataset registry. Each sample contains the node feature matrix, target physical fields, graph connectivity, and edge attributes. A training sample can be represented as
...
where \(\mathbf{X}\) is the node feature matrix, \(\mathbf{Y}\) is the target matrix of physical fields, \(\texttt{edge\_index}\) stores graph connectivity, and \(\texttt{edge\_attr}\) stores edge feature vectors.
...
where \(\mathbf{X}\) is the input matrix, \(N\) is the number of nodes, and \(F\) is the number of input features per node.

The target matrix is written as
...
where \(\mathbf{Y}\) is the target output matrix, \(N\) is the number of nodes, and \(C\) is the number of predicted physical fields.

The graph connectivity is represented as
...
where \(\texttt{edge\_index}\) is the edge index matrix, \(M\) is the number of graph edges, and each column stores the source and destination node indices of one edge.

The edge feature matrix is represented as
...
where \(\texttt{edge\_attr}\) is the edge attribute matrix, \(M\) is the number of graph edges, and \(F_e\) is the number of edge features.

The dataset is divided into training, validation, and test subsets. The training subset is used to update model parameters. The validation subset is used to monitor generalization during training and select the best checkpoint. The test subset is used only for final evaluation.

The model is optimized using the Adam optimizer. The parameter update rule can be written as
...
where \(\theta_t\) is the vector of trainable parameters at optimization step \(t\), \(\theta_{t+1}\) is the updated parameter vector, \(\eta\) is the learning rate, \(\hat{m}_t\) is the bias-corrected first moment estimate of the gradient, \(\hat{v}_t\) is the bias-corrected second moment estimate, and \(\epsilon\) is a small numerical constant used for stability.

Gradient clipping is applied to improve training stability. The clipped gradient is computed as
...
where \(\mathbf{g}\) is the gradient vector, \(\tau\) is the maximum allowed gradient norm, and \(\|\mathbf{g}\|_2\) is the Euclidean norm of the gradient.

The training loss at epoch \(e\) can be written as
...
where \(\mathcal{L}_{train}^{(e)}\) is the average training loss at epoch \(e\), \(\mathcal{D}_{train}\) is the training dataset, \(b\) is a training batch or sample, and \(\mathcal{L}^{(b)}\) is the loss computed for this batch or sample.

The validation loss is written as
...
where \(\mathcal{L}_{val}^{(e)}\) is the average validation loss at epoch \(e\), \(\mathcal{D}_{val}\) is the validation dataset, \(b\) is a validation batch or sample, and \(\mathcal{L}^{(b)}\) is the corresponding loss value.

The test loss is computed after training:
...
where \(\mathcal{L}_{test}\) is the final test loss, \(\mathcal{D}_{test}\) is the test dataset, \(b\) is a test batch or sample, and \(\mathcal{L}^{(b)}\) is the loss for this batch or sample.

Early stopping is used to stop training when the validation loss no longer improves. The stopping condition can be expressed as
...
In practice, training is stopped only if the validation loss does not improve for a specified number of epochs called patience. This prevents unnecessary training after the model has stopped improving and reduces the risk of overfitting.

During training, two checkpoints are saved. The last checkpoint stores the model state after the final epoch. The best checkpoint stores the model state with the lowest validation loss. The best checkpoint is usually used for final testing and inference because it corresponds to the strongest validation performance.

\subsection{Fine-Tuning Strategy}

Fine-tuning is included in the ConditionalMeshGraphNet design to make the model adaptable to new geological materials and new simulation scenarios. Training a full model from the beginning can be expensive, especially when the dataset is large or the architecture contains many message passing layers. Fine-tuning allows part of the previously learned representation to be reused.
...
where \(\theta\) is the full set of trainable model parameters, \(\theta_{enc}\) are encoder parameters, \(\theta_{proc}\) are processor parameters, and \(\theta_{dec}\) are decoder parameters.

In decoder-only fine-tuning, the encoder and processor are frozen, while only the decoder is trained:
...
where \(\theta_{enc}\) represents encoder parameters, and \(\text{frozen}\) means that these parameters are not updated during optimization.
...
where \(\theta_{proc}\) represents processor parameters, and \(\text{frozen}\) means that these parameters are not updated during optimization.
...
where \(\theta_{dec}\) represents decoder parameters, and \(\text{trainable}\) means that these parameters are updated during optimization.

Decoder-only fine-tuning is useful when the spatial interaction pattern learned by the encoder and processor is still valid, but the output mapping needs adaptation. For example, this mode may be used when the same type of mesh and physics are used, but the output field scaling or target selection changes.

In processor-decoder fine-tuning, the encoder is frozen, while the processor and decoder are trained:
...
where \(\theta_{enc}\) represents encoder parameters, and \(\text{frozen}\) means that these parameters are kept fixed.
...
where \(\theta_{proc}\) represents processor parameters, and \(\text{trainable}\) means that these parameters are updated during training.
...
where \(\theta_{dec}\) represents decoder parameters, and \(\text{trainable}\) means that these parameters are updated during training.

Processor-decoder fine-tuning is useful when the low-level feature representation is still meaningful, but the physical interaction dynamics must be adapted. For example, if the mesh feature scheme remains the same but the material behavior changes, updating the message passing processor can help the model learn new propagation patterns.

\subsection{Advantages of MeshGraphNet for This Task}

MeshGraphNet has several advantages for the prediction of thermoelastic waves in geological media. First, it works directly with unstructured meshes. This is important because COMSOL simulations are based on finite-element discretization, where mesh nodes are not arranged as a regular grid.

Second, the graph representation preserves local topology. The edge structure describes which nodes are neighbors, and this allows the model to learn interactions that follow the finite-element mesh connectivity. This is more natural for the given simulation data than forcing the domain into a regular image format.

Third, message passing can model local physical interactions. Thermoelastic wave propagation is governed by local relationships between temperature, displacement, velocity, stress, strain, and material properties. The graph neural network learns these relationships from simulation data.

Fourth, the model supports conditional inputs. Since material parameters and scenario parameters are included in the node feature vector, the same architecture can be trained to handle different rocks, thermal loads, and simulation settings, as long as the feature schema is consistent.

Fifth, inference with a trained surrogate model is faster than running a full COMSOL simulation. This makes the method useful for rapid scenario analysis, visualization, and repeated prediction.

Sixth, the architecture supports fine-tuning. A model trained on one group of datasets can be adapted to new datasets without always retraining all parameters from scratch. This improves the practical extendability of the approach.

\subsection{Limitations}

Despite its advantages, the MeshGraphNet approach also has limitations. The first limitation is that the quality of the model depends directly on the quality and coverage of the COMSOL-generated dataset. If important physical regimes are absent from the training data, the model cannot reliably learn them.

The second limitation is extrapolation. Neural surrogate models usually perform best inside the range of scenarios observed during training. If the model is applied to much higher temperatures, different material parameters, or boundary conditions that were not represented in the dataset, prediction accuracy may decrease.

The third limitation is related to mesh topology. Although graph neural networks can work with unstructured meshes, different mesh resolutions or strongly different connectivity patterns require careful preprocessing. The feature schema, edge construction method, and normalization strategy must remain consistent.

The fourth limitation is error accumulation during multi-step rollout. If the model is used recursively to predict many future time steps, errors from earlier predictions can influence later inputs. This can lead to drift, especially for long horizons.

The fifth limitation is that physical constraints are learned implicitly. The model learns from numerical simulation data, but it does not strictly enforce conservation laws, boundary conditions, or constitutive equations during inference. Therefore, additional validation is required before using predictions in high-responsibility engineering decisions.

\subsection{Summary}

In this work, MeshGraphNet is used as a graph-based surrogate model for predicting thermoelastic physical fields in geological media. The finite-element mesh generated in COMSOL is represented as a graph, where nodes correspond to mesh points or mesh elements, and edges describe local spatial connectivity between neighboring regions. Each node contains coordinates, material parameters, scenario parameters, and the current physical state. Each edge contains geometric information describing the relative position and distance between neighboring nodes.

The implemented ConditionalMeshGraphNet follows the Encoder--Processor--Decoder architecture. The encoder maps node and edge features into a latent space. The processor performs several message passing iterations, where edge updates, message aggregation, and node updates approximate local physical interactions in the discretized medium. The decoder converts the final latent node states into predicted physical fields.

The loss function is based on WeightedFieldMSE, which allows different physical groups, such as temperature, displacement, velocity, stress, strain, and other fields, to contribute differently to the optimization objective. The model is evaluated using global metrics, such as \(\mathrm{MSE}\), \(\mathrm{RMSE}\), and \(\mathrm{MAE}\), as well as per-field metrics, such as \(\mathrm{RMSE}_c\) and \(\mathrm{MAE}_c\). Fine-tuning strategies make the approach extendable to new rocks, new materials, and new simulation scenarios. Overall, MeshGraphNet provides a physically meaningful and computationally efficient method for approximating thermoelastic wave propagation on unstructured COMSOL meshes. 
...
where \(\hat{\mathbf{y}}_i^{t+\Delta t}\) is the predicted output vector for node \(i\) at time \(t+\Delta t\), \(f_{\theta}\) is the trainable ConditionalMeshGraphNet model, \(\theta\) denotes all trainable parameters of the model, \(\mathbf{x}_i^t\) is the input feature vector of node \(i\) at time \(t\), \(\mathcal{G}\) is the graph representation of the COMSOL mesh, and \(\Delta t\) is the time interval between the current and predicted states.
...
where \(\hat{\mathbf{Y}}^{t+\Delta t}\) is the predicted matrix of physical fields for all graph nodes at time \(t+\Delta t\), \(\mathbf{X}^{t}\) is the input node feature matrix at time \(t\), \(f_{\theta}\) is the neural network surrogate model, and \(\mathcal{G}\) is the graph structure of the finite-element mesh.
...
=
\begin{bmatrix}
(\hat{\mathbf{y}}_1^{t+\Delta t})^{\top} \\
(\hat{\mathbf{y}}_2^{t+\Delta t})^{\top} \\
\vdots \\
(\hat{\mathbf{y}}_N^{t+\Delta t})^{\top}
\end{bmatrix}
\]

where \(\hat{\mathbf{Y}}^{t+\Delta t}\) is the full predicted output matrix, \(\hat{\mathbf{y}}_i^{t+\Delta t}\) is the predicted physical field vector for node \(i\), and \(N\) is the total number of graph nodes.
...
where \(\hat{\mathbf{q}}_i^{t+\Delta t}\) is the predicted physical state of node \(i\) at time \(t+\Delta t\), \(f_{\theta}\) is the trainable graph neural network, \(\mathbf{x}_i^t\) is the current input feature vector of node \(i\), and \(\mathcal{G}\) is the graph representation of the mesh.
...
where \(\Delta \hat{\mathbf{q}}_i^t\) is the predicted change of the physical state at node \(i\), \(f_{\theta}\) is the trainable ConditionalMeshGraphNet model, \(\mathbf{x}_i^t\) is the input feature vector at time \(t\), and \(\mathcal{G}\) is the graph structure of the finite-element mesh.
...
where \(\hat{\mathbf{q}}_i^{t+\Delta t}\) is the reconstructed prediction of the physical state at the next time step, \(\mathbf{q}_i^t\) is the current physical state of node \(i\), and \(\Delta \hat{\mathbf{q}}_i^t\) is the predicted increment of the state.
...
where \(\mathbf{X}^{t}\) is the node feature matrix at time \(t\), \(N\) is the number of graph nodes, and \(F_v\) is the number of input node features.
...
where \(\mathbf{A}\) is the edge feature matrix, \(M\) is the number of graph edges, and \(F_e\) is the number of edge features.
...
where \(\hat{\mathbf{Y}}^{t+\Delta t}\) is the predicted output matrix at time \(t+\Delta t\), \(N\) is the number of graph nodes, and \(C\) is the number of predicted physical fields.
...
where \(\mathbf{h}_i^{0}\) is the initial latent representation of node \(i\), \(\phi_v^{enc}\) is the node encoder network, and \(\mathbf{x}_i^t\) is the input feature vector of node \(i\) at time \(t\).
...
where \(\mathbf{H}^{0}\) is the matrix of initial latent node representations, \(\phi_v^{enc}\) is the node encoder, and \(\mathbf{X}^{t}\) is the input node feature matrix.
...
where \(\mathbf{H}^{0}\) is the latent node representation matrix, \(\mathbf{h}_i^{0}\) is the initial latent vector of node \(i\), and \(N\) is the total number of nodes.
...
where \(\mathbf{e}_{ij}^{0}\) is the initial latent representation of edge \((i,j)\), \(\phi_e^{enc}\) is the edge encoder network, and \(\mathbf{a}_{ij}\) is the raw edge feature vector.
...
where \(\phi(\mathbf{z})\) is the output of the multilayer perceptron, \(\mathbf{z}\) is the input vector, \(L\) is the number of linear layers, \(\mathbf{W}_{\ell}\) is the weight matrix of layer \(\ell\), \(\mathbf{b}_{\ell}\) is the bias vector of layer \(\ell\), and \(\psi(\cdot)\) is the nonlinear activation function.
...
where \(\tilde{\mathbf{e}}_{ij}^{k+1}\) is the proposed update for edge \((i,j)\), \(\phi_e^{k}\) is the edge update network at message passing step \(k\), \(\mathbf{h}_i^k\) is the latent representation of the source node \(i\), \(\mathbf{h}_j^k\) is the latent representation of the destination node \(j\), and \(\mathbf{e}_{ij}^k\) is the current latent representation of edge \((i,j)\).
...
where \(\mathbf{e}_{ij}^{k}\) is the previous edge representation and \(\tilde{\mathbf{e}}_{ij}^{k+1}\) is the learned edge update.
...
where \(\mathbf{e}_{ij}^{k+1}\) is the updated edge representation, \(\operatorname{LayerNorm}\) is the layer normalization operation, \(\mathbf{e}_{ij}^{k}\) is the previous edge representation, and \(\tilde{\mathbf{e}}_{ij}^{k+1}\) is the computed edge update.
...
where \(\mathbf{m}_j^{k+1}\) is the aggregated message received by node \(j\), \(\mathbf{e}_{ij}^{k+1}\) is the updated edge representation from node \(i\) to node \(j\), and the summation is taken over all incoming edges \((i,j)\) connected to node \(j\).
...
where \(\tilde{\mathbf{h}}_j^{k+1}\) is the proposed update for node \(j\), \(\phi_v^{k}\) is the node update network at message passing step \(k\), \(\mathbf{h}_j^k\) is the previous latent representation of node \(j\), and \(\mathbf{m}_j^{k+1}\) is the aggregated message received from neighboring nodes.
...
where \(\mathbf{h}_j^k\) is the previous node representation and \(\tilde{\mathbf{h}}_j^{k+1}\) is the learned node update.
...
where \(\mathbf{h}_j^{k+1}\) is the updated latent representation of node \(j\), \(\operatorname{LayerNorm}\) is the normalization operation, \(\mathbf{h}_j^k\) is the previous node representation, and \(\tilde{\mathbf{h}}_j^{k+1}\) is the learned node update.

Residual connections help preserve information from previous representations and improve gradient flow during training. Layer normalization stabilizes the distribution of latent features and makes training more robust. This is important because the model performs several message passing steps, and without stabilization, the latent representations may become difficult to optimize.
\subsubsection{Decoder}
...
where \(\hat{\mathbf{Y}}\) is the predicted output matrix for all graph nodes, \(\hat{\mathbf{y}}_i\) is the predicted physical field vector for node \(i\), and \(N\) is the total number of graph nodes.
...
where \(\rho\) is the material density, \(\mathbf{u}\) is the displacement vector, \(t\) is time, \(\boldsymbol{\sigma}\) is the stress tensor, and \(\mathbf{b}\) is the body force vector.
...
where \(\mathbf{u}\) is the displacement vector, and \(u\), \(v\), and \(w\) are displacement components along the three spatial directions.
...
where \(\boldsymbol{\sigma}\) is the stress tensor, \(\sigma_x\), \(\sigma_y\), and \(\sigma_z\) are normal stress components, and \(\sigma_{xy}\), \(\sigma_{xz}\), and \(\sigma_{yz}\) are shear stress components.
...
where \(\boldsymbol{\varepsilon}\) is the strain tensor, \(\mathbf{u}\) is the displacement vector, \(\nabla \mathbf{u}\) is the displacement gradient tensor, and \((\nabla \mathbf{u})^{\top}\) is its transpose.
...
\boldsymbol{\varepsilon}
=
\boldsymbol{\varepsilon}^{mech}
+
\boldsymbol{\varepsilon}^{th}
\]

where \(\boldsymbol{\varepsilon}\) is the total strain tensor, \(\boldsymbol{\varepsilon}^{mech}\) is the mechanical strain tensor, and \(\boldsymbol{\varepsilon}^{th}\) is the thermal strain tensor.
...
where \(\boldsymbol{\varepsilon}^{th}\) is the thermal strain tensor, \(\alpha\) is the coefficient of thermal expansion, \(T\) is the current temperature, \(T_0\) is the reference temperature, and \(\mathbf{I}\) is the identity tensor.
...
The model is trained as a supervised regression model. The predicted physical fields are compared with target fields obtained from COMSOL simulations. A standard mean squared error loss is written as
...
where \(\mathcal{L}_{MSE}\) is the mean squared error loss, \(N\) is the number of nodes, \(C\) is the number of predicted physical fields, \(\hat{y}_{i,c}\) is the predicted value of field \(c\) at node \(i\), and \(y_{i,c}\) is the corresponding target value from the COMSOL simulation.
...
where \(\mathbf{X}\) is the node feature matrix, \(\mathbf{Y}\) is the target matrix of physical fields, \(\texttt{edge\_index}\) is the graph connectivity matrix, and \(\texttt{edge\_attr}\) is the matrix of edge features.
...
where \(\mathbf{X}\) is the node feature matrix, \(N\) is the number of nodes, and \(F_v\) is the number of node features.
...
where \(\mathbf{Y}\) is the target matrix, \(N\) is the number of nodes, and \(C\) is the number of predicted physical fields.
...
The model parameters are optimized using the Adam optimizer. The parameter update can be written as
...
where \(\theta_t\) is the parameter vector at optimization step \(t\), \(\theta_{t+1}\) is the updated parameter vector, \(\eta\) is the learning rate, \(\hat{m}_t\) is the bias-corrected first moment estimate of the gradient, \(\hat{v}_t\) is the bias-corrected second moment estimate of the gradient, and \(\epsilon\) is a small constant used for numerical stability.
...
The average training loss for epoch \(e\) is written as
...
Early stopping is used to stop training when the validation loss does not improve. The condition for no improvement at epoch \(e\) can be written as
...
In practice, training is stopped when this condition remains true for a predefined number of epochs called patience. This prevents unnecessary training and reduces the risk of overfitting. During training, two checkpoints are saved. The last checkpoint stores the final model state, while the best checkpoint stores the model state with the lowest validation loss.
...
where \(\mathcal{S}_{COMSOL}\) denotes the numerical simulation operator represented by COMSOL, \(f_{\theta}\) denotes the trained MeshGraphNet surrogate model, \(\mathbf{X}^{t}\) is the input node feature matrix at time \(t\), and \(\mathcal{G}\) is the graph representation of the finite-element mesh.
...
where \(\mathcal{D}_{train}\) is the training dataset, \(\mathbf{X}_n\) is the input feature matrix of sample \(n\), \(\mathbf{Y}_n\) is the target matrix of sample \(n\), \(\mathcal{G}_n\) is the graph structure of sample \(n\), and \(N_s\) is the number of training samples.
...
where \(\mathbf{X}_{new}\) is the input feature matrix for a new sample, \(\mathcal{G}_{new}\) is the graph structure for the new sample, and \(\mathcal{D}_{train}\) represents the distribution of training data.
...
\[
\hat{\mathbf{q}}^{t+(n+1)\Delta t}
=
f_{\theta}
\left(
\hat{\mathbf{q}}^{t+n\Delta t},
\mathcal{G}
\right)
\]

where \(\hat{\mathbf{q}}^{t+(n+1)\Delta t}\) is the predicted physical state at rollout step \(n+1\), \(f_{\theta}\) is the trained surrogate model, \(\hat{\mathbf{q}}^{t+n\Delta t}\) is the predicted physical state from the previous rollout step, and \(\mathcal{G}\) is the graph structure.
...
=
\arg\min_{\theta}
\mathbb{E}_{(\mathbf{X},\mathbf{Y},\mathcal{G})\sim\mathcal{D}_{train}}
\left[
\mathcal{L}
\left(
f_{\theta}(\mathbf{X},\mathcal{G}),
\mathbf{Y}
\right)
\right]
\]

where \(\theta^{*}\) is the optimized parameter set, \(\theta\) is the model parameter set, \(\mathcal{D}_{train}\) is the training data distribution, \(\mathbf{X}\) is the input feature matrix, \(\mathbf{Y}\) is the target matrix, \(\mathcal{G}\) is the graph representation of the mesh, \(f_{\theta}\) is the model prediction, and \(\mathcal{L}\) is the training loss function.
...
where \(\mathbf{x}_i^t\) is the input feature vector of node \(i\) at time \(t\), \(\mathbf{p}_i\) is the coordinate vector, \(\mathbf{m}_i\) is the material parameter vector, \(\mathbf{s}\) is the scenario parameter vector, and \(\mathbf{q}_i^t\) is the current physical state vector.
...
where \(\hat{\mathbf{Y}}^{t+\Delta t}\) is the predicted matrix of physical fields, \(f_{\theta}\) is the trained ConditionalMeshGraphNet model, \(\mathbf{X}^{t}\) is the current node feature matrix, and \(\mathcal{G}\) is the graph structure.
